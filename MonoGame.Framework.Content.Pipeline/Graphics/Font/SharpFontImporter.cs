    ...
    internal class SharpFontImporter : IFontImporter {
        ...
        public void Import(FontDescription options, string fontName) {
            ...
            try {
                ...
                //List with metrics for each glyph: height above the baseline, height below the baseline, total height
                List<Tuple<char, uint,int, int, int>> glyphMetrics = new List<Tuple<char,uint, int, int, int>>();
                //Tuple<char, uint,int, int, int> glyphMetric;
                var glyphList = new List<Glyph>();
                for(uint i = 0; i < face.GlyphCount; i++) {
                    face.LoadGlyph(i, LoadFlags.Default, LoadTarget.Normal);
                    glyphMetrics.Add(new Tuple<char, uint, int, int, int>(
                        (char)i,
                        i,
                        face.Glyph.Metrics.HorizontalBearingY >> 6,
                        (face.Glyph.Metrics.Height - face.Glyph.Metrics.HorizontalBearingY) >> 6,
                        face.Glyph.Metrics.Height >> 6
                    ));
                    var found = glyphIndexes.FindIndex(p => p.Value == i);
                    if(found !=-1) {
                        glyphList.Add(ImportGlyph(face,glyphIndexes[found].Key));
                    }
                }

                // Rasterize each character in turn.
                //foreach(char character in characters) {
                //    var glyph = ImportGlyph(character, face, out glyphMetric);
                //    glyphList.Add(glyph);
                //    glyphMetrics.Add(glyphMetric);
                //}
                Glyphs = glyphList;
                ...
                int maxAscender = glyphMetrics.Max(t => t.Item3);
                int maxDescender = glyphMetrics.Max(t => t.Item4);
                int maxCharHeight = glyphMetrics.Max(t => t.Item5);
                int maxHeightNeeded = maxAscender + maxDescender;
                if(maxHeightNeeded > face.Size.Metrics.Height>>6) {
                    LineSpacing = maxHeightNeeded;
                    YOffsetMin = -maxAscender;
                } else {

                    LineSpacing = face.Size.Metrics.Height>>6;
                    YOffsetMin = -face.Size.Metrics.Ascender>>6;
                }
                //YOffsetMin = -maxAscender - (maxHeightNeeded - maxCharHeight) / 2;
                //YOffsetMin = -(face.Size.Metrics.Ascender >> 6) + (((int) LineSpacing - maxHeightNeeded) / 2);
                //YOffsetMin = -face.Size.Metrics.Ascender >> 6;
            } finally {
                if(face != null)
                    face.Dispose();
        ...
        private Glyph ImportGlyph(Face face, char character) {
        ...
        private Glyph ImportGlyph(char character, Face face, out Tuple<char, int, int, int> glyphMetric) {
            ...
            abc.B = face.Glyph.Metrics.Width >> 6;
            abc.C = (face.Glyph.Metrics.HorizontalAdvance >> 6) - (abc.A + abc.B);
            glyphMetric = new Tuple<char, int, int, int>(
                character,
                face.Glyph.Metrics.HorizontalBearingY >> 6,
                (face.Glyph.Metrics.Height - face.Glyph.Metrics.HorizontalBearingY) >> 6,
                face.Glyph.Metrics.Height >> 6
